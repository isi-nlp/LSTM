...
int main (int argc, char *argv[]) 
{
  ...
  //NOW TO PRINT OUT THE HIDDEN STATE
  for (size_t i =0;i<current_minibatch_size;i++){
    cerr<<"Hidden state is ";
      for (size_t j=0;j<myParam.output_embedding_dimension-1;j++){
        cerr<<prop.second_hidden_activation_node.fProp_matrix(j,i)<<" ";
      }
      cerr<<prop.second_hidden_activation_node.fProp_matrix(myParam.output_embedding_dimension,i)<<endl;
  }
  ...
  //I NEED TO PRINT OUT THE SCORES OF THE OUTPUT WORD. FPROP MATRIX HAS SIZE
  //VOCAB_SIZE*MINIBATCH_SIZE
  for (size_t i =0;i<current_minibatch_size;i++){
    size_t word = minibatch(myParam.ngram_size-1,i);
    cerr<<"Output word is "<<word<<" and score for the word is "<<scores(word,i)<<endl;
  }
